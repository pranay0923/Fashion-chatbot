...
class ImageAnalysisService:
    ...
    def analyze_fashion_image(self, image_path_or_base64, user_query=None):
        ...
        try:
            ...
            if str(image_path_or_base64).startswith('data:image'):
    ...
    def extract_colors_from_image(self, image_path):
        ...
        try:
            ...
            if colors:
                ...
                total_counts = sum([c[0] for c in colors])
        ...
        if user_input.lower() == 'quit':
            ...
            if images:
                for i, img in enumerate(images, 1):
                    # img structure: (id, user_id, image_path, image_description, detected_items, color_analysis, style_analysis, upload_timestamp)
                    print(f"{i}. {img[3]} (Uploaded: {img[7] if len(img) > 7 else 'N/A'})")
            else:
